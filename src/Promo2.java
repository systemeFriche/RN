...
/*public void ajoutNoteModuleEtu(SpreadsheetDocuments spreadsheetDocumentsRN, XSpreadsheet xSheet, XSpreadsheet xSheetELP, int codeEtu, String codeModule) {
    	...
    	noteRN=spreadsheetDocumentsRN.readFloatCellule(xSheet, 7, ligneRN);
    	
    	//si la note module a été calculée
    	if(!(noteRNString.equals(""))){
    		//récupération note Module APOGEE	
    		noteApogee=spreadsheetDocumentsRN.readFloatCellule(xSheetELP, colonneModule, ligneEtu);
    ...
    /*public float getNoteSportRN(SpreadsheetDocuments spreadsheetDocuments, XSpreadsheet xSheet, int codeEtu){
    	...
    	if(noteString.equals("")){	
        	...
        	note=spreadsheetDocuments.readFloatCellule(xSheet,colonneNoteSport,ligneEtu);    		
    	}
					...
					if(resultsSousModule==null){
						...
						if(resultsEpreuve!=null){
							for(JsonObject resultEpreuve : resultsEpreuve.getValuesAs(JsonObject.class)){
								Epreuve2 epreuve1 = new Epreuve2();
	...
	public void verificationMoyennesPromo() {
		...
		Ue ue;
		List<Ue> listeUe;// = new ArrayList<Ue>();
		List<Module2> listeModulesUe;// = new ArrayList<Module2>();
		
		Module2 module,sousModule;
		int nbrEtu=this.getNbrEtu();
		...
		for(i=0;i<listeUe.size()-1;i++){
			...
			for(j=0;j<listeModulesUe.size();j++){
				...
				if(module.getListeElements().get(0) instanceof Module2){
					//on parcourt tous les sous-modules de ce module
					for(k=0;k<module.getListeElements().size();k++){
						sousModule=(Module2) module.getListeElements().get(k);
			...
			for(j=0;j<listeModulesUe.size();j++){
				...
				if(module.getListeElements().get(0) instanceof Module2){
					...
					for(k=0;k<module.getListeElements().size();k++){
						...
						for(l=0;l<sousModule.getListeElements().size();l++){
							...
							epreuve=(Epreuve2) sousModule.getListeElements().get(l);
				...
				//le module contient des épreuves
					...
					for(l=0;l<module.getListeElements().size();l++){
						...
						//on récupère l'épreuve
						epreuve=(Epreuve2) module.getListeElements().get(l);
	...
	public void generationRnEtudiant(SpreadsheetDocuments spreadsheetDocumentsRn,String cheminFichierRn,String cheminFichierRnEtuOdsBase,String cheminFichierRnEtuPdfBase){
		...
		//ATTENTION SAVE AS OU SAVE ? CLOSE xDocRn ? CLOSE FEUILLE ? 
